...
const StepOne = ({ formData, setFormData, next, formErrors }) => {
	...
	const handleDateChange = (date) => {
		// date ko YYYY-MM-DD format me convert karna
		const formattedDate = date ? date.toISOString().split("T")[0] : "";
		setFormData({ ...formData, date_of_birth: formattedDate });
		setDateOfBirth(date); // local state bhi update kar dena
	};
	...
	const handleSubmit = (e) => {
		e.preventDefault();
		if (formData.phone.lenght < 10 || formData.phone.lenght > 15) {
